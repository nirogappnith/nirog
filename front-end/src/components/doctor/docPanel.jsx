...
const DoctorPanel = () => {
  ...
  const getPatients = async (doctor_id) => {
    try {
      const response = await fetch(
        "https://localhost:8008/doctor/getPatients",
        {
          method: "POST",
          headers: {
  ...
  const getDoctorDetails = async (doctor_id) => {
    try {
      const response = await fetch("https://localhost:8008/doctor/getDoctor", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleOFF = async () => {
    try {
      ...
      const response = await fetch(
        "https://localhost:8008/doctor/doneForToday",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <button
        onClick={handleOFF}
        type="button"
        class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mt-3 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"
      >
        Done For today
      </button>
    </div>
