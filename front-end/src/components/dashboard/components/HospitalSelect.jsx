...
const HospitalSelect = ({changeMode, setHospital}) => {
  ...
  useEffect(()=> {
    const fetchhospital = async () => {
      await axios.get('https://localhost:8008/admin/getHospital')
        .then(res=>{
  ...
  const handleSelect = (value) => {
    ...
    const saveHospital = async () => {
      await axios.post('https://localhost:8008/user/add_hospital', {userJWT, hospName: currentHospital})
        .then(res=> {
          if(res.status == 200) {
        ...
        {hospitals.map((hospital, index) => (
          <div key={index} className="flex border-black bg-slate-200 border-2 w-11/12 m-auto rounded-3xl items-center justify-between px-5 py-2">
            <div className="flex flex-col items-start">
              ...
              <img src="/location.png" alt="Location icon" className="mr-0" style={{ width: '3em', height: '3em' , marginTop: '-0.50em'}} />
                <p>
                  {hospital.address}
                </p>
              </div>
